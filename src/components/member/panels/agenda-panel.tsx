      ...
      {items.map((item) =>
        item.kind === "evento" ? (
          ...
          >
            ...
            <div className="mt-1 text-[11px] capitalize text-gray-500">{formatDateTimeBR(item.date)}</div>
            <div className="mt-2 text-xs font-bold text-na-green">
          ...
          <div
            ...
            className="rounded-xl border border-na-green/30 bg-na-green-light/40 p-3"
          >
            <div className="mb-1 flex items-center gap-2 text-[11px] font-semibold text-na-green-dark">
              <GraduationCap size={14} /> {ACTIVITY_LABEL[item.type] ?? item.type} · {item.className}
            </div>
            <div className="text-sm font-semibold text-gray-900">{item.title}</div>
            <div className="mt-1 text-[11px] text-gray-500">Data: {formatDateTimeBR(item.date)}</div>
            {item.studyItems && (
              <div className="mt-2 rounded-lg bg-white p-2 text-[11px] text-gray-700">
